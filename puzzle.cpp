...
vector<vector<vector<int>>> vecinos(vector<vector<int>> a)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++) // asignacion de posiciones
		{
			if (a[i][j] == 0)
			{
				pos.first = i;
...
void encontrarSolucion(vector<vector<int>> a, int moves)
{
	...
	while (!colaP.empty())
	{
		// proceso de solución del puzzle
...
// Método para saber si el puzzle es resoluble
...
// Metodo que compara los numeros repetidos
...
// Generacion aleatoria de estados iniciales del puzzle
int *ejemplosPuzzle(int arreglo[])
{
	...
	for (int i = 0; i < 9; i++)
	{
		...
		{
			num = rand() % 9;
		} while (repeticionDominio(num, arreglo)); // asignacion de numeros aleatorios
		arreglo[i] = num;
	}
	return arreglo;
}
...
int main()
{
	...
	for (int i = 0; i < 3; i++)
	{ // Asignación del estado generado aleatoriamente al estado inicial
		for (int j = 0; j < 3; j++)
		{
	...
	if (distanciaManhattan <= 10)
	{
		...
		cout << "El valor heuristico es de " << distanciaManhattan << endl; // Output dato del valor heurístico
		if (existeSolucion(obtenerInversiones(arregloAux)))
		{
			// Verificación si existe solución
			...
			cout << endl;
			encontrarSolucion(estadoInicial, 0);				   // Resolución del puzzle
			cout << "Distancia Manhattan: " << distanciaManhattan; // Output distancia Manhattan
			cout << endl;
			cout << "Costo de cada nodo: "; // Output costo de cada nodo
			for (int idx : valoresNOdos)
			{
				cout << idx << " ";
			}
			cout << endl;
			cout << "Profundidad: " << profundidad << endl; // Output dato de profundidad de la solución
		}
		else
